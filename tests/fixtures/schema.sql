...
CREATE TABLE `home_slider_items` (
  ...
  `sort_order` int(10) unsigned NOT NULL DEFAULT 0 COMMENT 'ترتيب الصورة داخل نفس الشريحة (من اليسار لليمين)',
...
CREATE TABLE `home_sliders` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `sort_order` int(10) unsigned NOT NULL DEFAULT 0 COMMENT 'ترتيب ظهور الشريحة بالكاروسيل — يُعاد بناؤه بالكامل عند كل حفظ',
  `updated_by_admin_id` int(10) unsigned DEFAULT NULL COMMENT 'آخر أدمن قام بالحفظ الكامل للسلايدر (لكل الصفحة وليس فقط هذه الشريحة)',
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
...
CREATE TABLE `order_expiry_log` (
  ...
  `previous_admin_id` int(10) unsigned DEFAULT NULL COMMENT 'الأدمن الذي كان قد أخذ الطلب قبل انتهاء المهلة',
